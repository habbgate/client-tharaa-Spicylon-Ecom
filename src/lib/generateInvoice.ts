...
export const generateInvoice = async (order: any) => {
  ...
  doc.text(`Date: ${new Date(order.createdAt).toLocaleDateString()}`, metaX, metaStartY + lineH, { align: "right" });

  // Status pill
  const status = order.isPaid ? "PAID" : "PENDING";
  const pillColor: [number, number, number] = order.isPaid ? [22, 163, 74] : [234, 88, 12];
  doc.setFillColor(...pillColor);
  doc.roundedRect(metaX - 18, metaStartY + lineH * 2 - 4, 22, 6.5, 1.5, 1.5, "F");
  doc.setFontSize(7);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(255, 255, 255);
  doc.text(status, metaX - 7, metaStartY + lineH * 2 + 0.8, { align: "center" });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(80, 80, 80);
  doc.text(`Payment: ${order.paymentMethod || "Stripe"}`, metaX, metaStartY + lineH * 3, { align: "right" });
  ...
  if (addr.address)          { doc.text(addr.address, 14, addrY); addrY += 6; }
  if (addr.city || addr.postalCode) { doc.text(`${addr.city || ""}, ${addr.postalCode || ""}`, 14, addrY); addrY += 6; }
  if (addr.country)          { doc.text(addr.country, 14, addrY); addrY += 6; }
  if (addr.phone)            { doc.text(`Phone: ${addr.phone}`, 14, addrY); addrY += 6; }
  ...
  const subtotal = order.orderItems.reduce(
    (acc: number, item: any) => acc + item.quantity * item.price, 0,
  );
  ...
  doc.text(`${order.currency || "USD"} ${subtotal.toFixed(2)}`, totalsX, totY, { align: "right" });

  totY += 7;
  doc.text("Shipping:", labelX, totY);
  doc.text(`${order.currency || "USD"} ${shippingPrice.toFixed(2)}`, totalsX, totY, { align: "right" });

  if (taxPrice > 0) {
    totY += 7;
    doc.text("Tax:", labelX, totY);
    doc.text(`${order.currency || "USD"} ${taxPrice.toFixed(2)}`, totalsX, totY, { align: "right" });
  }

  // Total row with background
  totY += 9;
  doc.setFillColor(15, 15, 15);
  doc.roundedRect(labelX - 4, totY - 5.5, totalsX - labelX + 4 + 4, 10, 2, 2, "F");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.setTextColor(255, 255, 255);
  doc.text("TOTAL", labelX, totY + 1.5);
  doc.setTextColor(234, 88, 12);
  doc.text(`${order.currency || "USD"} ${totalPrice.toFixed(2)}`, totalsX, totY + 1.5, { align: "right" });
  ...
  doc.setFontSize(8);
  doc.setTextColor(160, 160, 160);
  doc.text("Thank you for shopping with Spicylon!  ·  spicylon.com", PW / 2, 291, { align: "center" });
